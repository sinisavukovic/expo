...
public class NotificationChannelSerializer {
  ...
  public static Bundle toBundle(@Nullable NotificationChannel channel) {
    ...
    result.putInt(IMPORTANCE_KEY, channel.getImportance());
    ...
    result.putInt(LOCKSCREEN_VISIBILITY_KEY, channel.getLockscreenVisibility());
  ...
  public static Bundle toBundle(AudioAttributes attributes) {
    Bundle result = new Bundle();
    result.putInt(AUDIO_ATTRIBUTES_USAGE_KEY, attributes.getUsage());
    result.putInt(AUDIO_ATTRIBUTES_CONTENT_TYPE_KEY, attributes.getContentType());
